...
impl InputBuffers {
    fn new(line: Vec<String>, file_name: Option<String>) -> Self {
        InputBuffers {
            content: line,
            file_name,
        }
...
pub struct Config {
    ...
    pub input_lines: Vec<InputBuffers>,
    pub flags: Flags,
}
impl Config {
    #[allow(unused)]
...
fn args_flags(input: &[String]) -> Result<(Flags, Vec<String>), String> {
    ...
    return Ok((flags, args));
}

fn read_input(args: Vec<String>) -> Result<Vec<InputBuffers>, String> {
    ...
    if !args[1..].is_empty() {
        for arg in args[1..].iter() {
            ...
            let line: Vec<String> = file_content
                ...
                .map(|x| x.to_string())
                .collect();
            let input_line = InputBuffers::new(line, Some(arg.clone()));
    ...
    let input_line = InputBuffers::new(line, None);
    lines.push(input_line);
    return Ok(lines);
}
...
pub fn pattern_parser(pattern: &str) -> Vec<Vec<String>> {
    ...
    for pat in patterns {
        let tokenized_pattern = tokenize_pattern(pat);

        let compiled_pattern = compile_pattern(tokenized_pattern);
...
fn tokenize_pattern(pattern: String) -> Vec<String> {
    ...
    for (index, &i) in pattern.iter().enumerate() {
        if i == r#"\"# {
            distinguish_marks(&mut tokenized_pattern, &mut temp);
            slash_flag = true;
